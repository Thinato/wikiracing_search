...
# Elementos da Graphical User Interface - GUI
menu = pg_gui.elements.UIWindow(pg.Rect(400, 0, 400, 400), window_display_title="Menu")

lbl_start = pg_gui.elements.UILabel(
    pg.Rect(10, 10, 110, 32), container=menu, text="Pág. Inicial:"
)
txt_start = pg_gui.elements.UITextEntryLine(pg.Rect(120, 10, 240, 32), container=menu)
sel_start = pg_gui.elements.UISelectionList(
    pg.Rect(120, 40, 240, 100), container=menu, item_list=[], visible=0
)
start_last_search = ""

lbl_final = pg_gui.elements.UILabel(
    pg.Rect(10, 42, 110, 32), container=menu, text="Pág. Final:"
)
txt_final = pg_gui.elements.UITextEntryLine(pg.Rect(120, 42, 240, 32), container=menu)
sel_final = pg_gui.elements.UISelectionList(
    pg.Rect(120, 72, 240, 100), container=menu, item_list=[], visible=0
)
final_last_search = ""

lbl_algorithm = pg_gui.elements.UILabel(
    pg.Rect(10, 74, 110, 32), container=menu, text="Algorítmo:"
)
cmb_algorithm = pg_gui.elements.UIDropDownMenu(
    relative_rect=pg.Rect(120, 74, 240, 32),
    container=menu,
    starting_option="DFS",
    options_list=["DFS", "IDS", "BFS"],
)


btn_create_nodes = pg_gui.elements.UIButton(
    pg.Rect(10, 290, 150, 32), text="Criar Nós", container=menu
)
# btn_create_nodes.disable()
btn_start_search = pg_gui.elements.UIButton(
    pg.Rect(170, 290, 150, 32), text="Iniciar Busca", container=menu
)
...
# função responsável por criar o grafo e pesquisar em forma de profundidade limitada
def create_graph(key: str, depth_limit: int = 4, index: int = 0) -> tuple((bool, int)):
    try:
        ...
        if hps:
            ...
            if response.ok:
                ...
                hps = [
                    unquote(hp.get("href")[6:])
                    for hp in list(
                        tree.xpath(
                            '//div[@class="mw-parser-output"]/p/a[@href and (@class!="new" or not(@class))]'
                        )
                    )
                ]
                cache[key] = hps
...
while True:
    # dt = delta time
    # é a diferença entre cada 'tick'
    dt = clock.tick() / 1000
    # muda o titulo da aplicação para o FPS atual
    ...
    for event in pg.event.get():
        if event.type == pg.QUIT:
            ...
            if event.ui_element == txt_start:
                ...
                response = get(
                    f"https://pt.wikipedia.org/w/rest.php/v1/search/title?q={txt_start.get_text()}&limit=10"
                )
                ...
                response = get(
                    f"https://pt.wikipedia.org/w/rest.php/v1/search/title?q={txt_final.get_text()}&limit=10"
                )
            ...
            if event.ui_element == btn_create_nodes:
                end_key = txt_final.get_text()
                match cmb_algorithm.selected_option:
                    case "DFS":
